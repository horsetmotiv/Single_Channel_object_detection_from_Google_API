...
class MobilenetV1Test(tf.test.TestCase):
  ...
  def testUnknownImageShape(self):
    ...
    input_np = np.random.uniform(0, 1, (batch_size, height, width, 1))
    with self.test_session() as sess:
      # inputs = tf.placeholder(tf.float32, shape=(batch_size, None, None, 3))
      inputs = tf.placeholder(tf.float32, shape=(batch_size, None, None, 1))
      logits, end_points = mobilenet_v1.mobilenet_v1(inputs, num_classes)
      self.assertTrue(logits.op.name.startswith('MobilenetV1/Logits'))
      self.assertListEqual(logits.get_shape().as_list(),
                           [batch_size, num_classes])
  ...
  def testUnknowBatchSize(self):
    ...
    images = tf.random_uniform((batch_size, height, width, 1))
  ...
  def testEvaluation(self):
    ...
    eval_inputs = tf.random_uniform((batch_size, height, width, 1))
  ...
  def testTrainEvalWithReuse(self):
    ...
    train_inputs = tf.random_uniform((train_batch_size, height, width, 1))
    mobilenet_v1.mobilenet_v1(train_inputs, num_classes)
    # eval_inputs = tf.random_uniform((eval_batch_size, height, width, 3))
    eval_inputs = tf.random_uniform((eval_batch_size, height, width, 1))
    logits, _ = mobilenet_v1.mobilenet_v1(eval_inputs, num_classes,
                                          reuse=True)
    predictions = tf.argmax(logits, 1)
  ...
  def testLogitsNotSqueezed(self):
    ...
    images = tf.random_uniform([1, 224, 224, 1])
