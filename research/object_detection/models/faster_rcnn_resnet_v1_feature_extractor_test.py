...
class FasterRcnnResnetV1FeatureExtractorTest(tf.test.TestCase):
  ...
  def test_extract_proposal_features_returns_expected_size(self):
    for architecture in ['resnet_v1_50', 'resnet_v1_101', 'resnet_v1_152']:
      ...
      #################################################################################
      preprocessed_inputs = tf.random_uniform(
          [4, 224, 224, 1], maxval=255, dtype=tf.float32)
  ...
  def test_extract_proposal_features_half_size_input(self):
    ...
    ##################################################################
    preprocessed_inputs = tf.random_uniform(
        [1, 112, 112, 1], maxval=255, dtype=tf.float32)
    rpn_feature_map = feature_extractor.extract_proposal_features(
        preprocessed_inputs, scope='TestScope')
    features_shape = tf.shape(rpn_feature_map)
  ...
  def test_extract_proposal_features_dies_on_very_small_images(self):
    ...
    #############################################################################
    preprocessed_inputs = tf.placeholder(tf.float32, (4, None, None, 1))
    rpn_feature_map = feature_extractor.extract_proposal_features(
        preprocessed_inputs, scope='TestScope')
    features_shape = tf.shape(rpn_feature_map)
    ...
    with self.test_session() as sess:
      ...
      with self.assertRaises(tf.errors.InvalidArgumentError):
        sess.run(
            ...
            ####################################################################
            feed_dict={preprocessed_inputs: np.random.rand(4, 32, 32, 1)})
  ...
  def test_extract_proposal_features_dies_with_incorrect_rank_inputs(self):
    ...
    ####################################################################
    preprocessed_inputs = tf.random_uniform(
        [224, 224, 1], maxval=255, dtype=tf.float32)
    with self.assertRaises(ValueError):
  ...
  def test_extract_box_classifier_features_returns_expected_size(self):
    ...
    with self.test_session() as sess:
      ...
      ####################################################################
      self.assertAllEqual(features_shape_out, [1, 7, 7, 2048])
